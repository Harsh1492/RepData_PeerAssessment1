...
withoutMissing <- dataset
for(r in 1:nrow(withoutMissing)){
  if (is.na(withoutMissing$steps[r])) {
    repl <- meanStepsPerInterval$steps[meanStepsPerInterval$interval == withoutMissing$interval[r]];
    withoutMissing$steps[r] <- repl;
  }
}
# we verify it worked
sum(is.na(dataset$steps))
sum(is.na(withoutMissing$steps))
str(withoutMissing$steps)
```

4.4. Make a histogram of the total number of steps taken each day and Calculate and report the mean and median total number of steps taken per day. 
```{r histogramMissingsEstimated, fig.height=6, fig.width=10}
library(lattice)
sumStepsPerDayNoMissing <- aggregate(steps ~ date, data=withoutMissing, sum)
